...
- Consumers run `npx skills add Paldom/node-skills` — the CLI
  ...
  users can also `gh skill install Paldom/node-skills <skill>
...
gh repo edit Paldom/node-skills --visibility public --accept-visibility-change-consequences
...
gh api repos/Paldom/node-skills/private-vulnerability-reporting --method PUT

# 3. Cut a versioned release (matches .claude-plugin/plugin.json) + topics
gh skill publish --tag v0.1.0
gh repo edit Paldom/node-skills --add-topic skills-sh

# 4. Verify installability exactly like a consumer
npx skills add Paldom/node-skills --list
mkdir -p /tmp/skills-verify && cd /tmp/skills-verify
npx skills add Paldom/node-skills --skill '*' -a claude-code -y
...
(`https://skills.sh/b/Paldom/node-skills`) linking to the repo's
skills.sh page.
